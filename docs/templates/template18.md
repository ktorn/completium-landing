...
It requires that:
* the reveal period duration be greater or equal to the buy period duration
...
```archetype
transition initialise(ob : date, cb : date, cr : date, t : nat, rf : rational) {
  called by owner
  require {
